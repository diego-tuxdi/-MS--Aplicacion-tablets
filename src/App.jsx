...
import { UserCircle2, KeyRound, AlertCircle, RefreshCw, MoveRight, Eye, EyeOff, LogOut, Truck, Package, Factory, MapPin, ChevronLeft, Wrench, CheckCircle2, Scissors, Zap, Monitor, Droplet, Hammer, Home, Menu as MenuIcon, Bell, MessageCircle } from 'lucide-react';
      ...
      try {
        ...
        const fetchedUsers = usersLines.slice(1).reduce((acc, line) => {
          ...
          if (cells.length > 7) {
            ...
            // Email is 3
            const codigo = cells[4].trim(); // Código is now 4
            const rol = cells[5]?.trim(); // Rol is now 5
            ...
            const strActivo = cells[7]?.trim().toUpperCase(); // Activo is now 7

            // Parse Modulos (Column 8). Si no está la columna, por precaución asignamos todos para no romper nada viejo.
            let userModules = ['logística', 'inventario', 'producción'];
            if (cells.length > 8) {
              ...
              if (modulosStr) {
                userModules = modulosStr.split(',').map(m => m.trim().toLowerCase()).filter(Boolean);
              } else {
                userModules = []; // Explícitamente vacío
              }
            }

            // Checkbox in Google sheets usually exports as TRUE/VERDADERO when checked
    ...
    try {
      ...
      if (user.code === code) {
        ...
        if (API_URL) {
          ...
          }).catch(console.error); // We don't await because we don't want to block login
        }
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setSuccess('Código de autorización reenviado correctamente. Revise su bandeja de entrada.');
    ...
    const renderContent = () => {
      switch (activeTab) {
        case 'inicio':
          ...
          const hasLogistica = modules.some(m => m.includes('log'));
          const hasInventario = modules.some(m => m.includes('inv'));
          const hasProduccion = modules.some(m => m.includes('prod'));
